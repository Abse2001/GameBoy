...
export default ({
  ...
  sharedSpiTracePaths = false,
  chainSharedSignalTraces = false,
  ...
  >
    ...
    {routeDisplayFirst && (
      <autoroutingphase name="DISPLAY_BUS_FIRST" phaseIndex={0} autorouter={router} />
    )}
    ...
    {routeRegulatorFirst && (
      <autoroutingphase name="MCU_REGULATOR_FIRST" phaseIndex={1} autorouter={router} />
    )}

    {routeDecouplingFirst && (
      <autoroutingphase name="MCU_DECOUPLING_FIRST" phaseIndex={1} autorouter={router} />
    )}
    ...
    <RP2350CompactLayout
      ...
      parentOwnsSharedSpiTraces={sharedSpiTracePaths}
      multiTerminalRunTrace={sharedSpiTracePaths}
      chainRunTrace={chainSharedSignalTraces}
      peripheralPlacements={mcuPeripheralPlacements}
      debugTestpointEscape={debugTestpointEscape}
      debugTestpointPlacements={mcuDebugTestpointPlacements}
    ...
    <Microcontroller_RP2350
      ...
      supplyRoutingPhase={routeSupplyNetsFirst ? (routeHighSpeedFirst ? 2 : 1) : undefined}
    ...
    <trace
      ...
      from={publishedMcuModule ? (mcuHeaders ? ".MCU .J_RIGHT > .pin22" : ".MCU .J_USB > .A4B9") : ".MCU .J_USB > .A4B9"}
      to="net.VBUS"
      {...powerTraceProps}
      {...vbusLabel}
    />
    <trace
      name="MCU_GND"
      from={publishedMcuModule ? (mcuHeaders ? ".MCU .J_RIGHT > .pin3" : ".MCU .MCU_CORE .U1 > .GND") : ".MCU .U1 > .GND"}
      to="net.GND"
      {...gndLabel}
    />
    <trace
      name="MCU_V3V3"
      from={publishedMcuModule ? (mcuHeaders ? ".MCU .J_RIGHT > .pin21" : ".MCU .MCU_CORE .U1 > .VREG_VIN") : feedMcuAtInputCap ? ".MCU .C_VREG_IN > .pin1" : ".MCU .U1 > .VREG_VIN"}
      to="net.V3V3"
      {...powerTraceProps}
      {...v3v3Label}
    />
    ...
    <SS34 name="D_USB_POWER" pcbX={-16 + usbDiodeOffset.x} pcbY={32 + usbDiodeOffset.y} schX={-29} schY={24} />
    ...
    <capacitor name="C_3V3_OUT" capacitance="1uF" maxDecouplingTraceLength="5.5mm" footprint="0603" supplierPartNumbers={{ jlcpcb: ["C15849"] }} pcbX={(ldoFlipped ? -20.1 : -17.9) + ldoOffset.x} pcbY={(ldoFlipped ? 15.5 : 22.5) + ldoOffset.y} pcbRotation={ldoFlipped ? 270 : 90} schX={-17} schY={24.5} {...ldoPlacements?.C_3V3_OUT} />
    ...
    <trace name="REG_3V3_INPUT_CAP" routingPhaseIndex={routeDecouplingFirst ? 1 : undefined} from=".C_3V3_IN > .pin1" to=".U_3V3 > .VIN" maxLength="5.5mm" />
    <trace
      ...
      maxLength="30mm"
    />
    <trace name="REG_3V3_OUTPUT_CAP" routingPhaseIndex={routeDecouplingFirst ? 1 : undefined} from=".C_3V3_OUT > .pin1" to=".U_3V3 > .VOUT" maxLength="5.5mm" />
    ...
    <trace name="AUDIO_SIGNAL" from={publishedMcuModule ? (mcuHeaders ? ".MCU .J_RIGHT > .pin16" : ".MCU .MCU_CORE .U1 > .GPIO12") : ".MCU .U1 > .GPIO22"} to="net.AUDIO_PWM" />
    ...
    {storage && publishedMcuModule && (
      <group name="STORAGE" schX={26} schY={32}>
        <MicroSDStorage name="SD" pcbX={34} pcbY={-33.7} pcbRotation={180} sdDetectEscape={sdDetectEscape} decouplingRoutingPhase={routeDecouplingFirst ? 1 : undefined} />
        {!sharedSpiTracePaths && !chainSharedSignalTraces && (
          <>
        ...
        <trace name="SD_CS" from=".SD .J_SD > .CS" to={mcuHeaders ? ".MCU .J_RIGHT > .pin2" : ".MCU .MCU_CORE .U1 > .GPIO1"} />
        <trace name="SD_CARD_DETECT" from=".SD .J_SD > .CD" to={mcuHeaders ? ".MCU .J_RIGHT > .pin19" : ".MCU .MCU_CORE .U1 > .GPIO14"} />
      </group>
    )}

    {chainSharedSignalTraces && storage && publishedMcuModule && !mcuHeaders && (
      <>
        <trace name="SD_SCK_CHAIN" from=".SD .J_SD > .CLK" to=".J_LCD .J_HEADER > .SCK" />
        <trace name="SD_MOSI_CHAIN" from=".SD .J_SD > .MOSI" to=".J_LCD .J_HEADER > .SDI_MOSI" />
        <trace name="SD_MISO_CHAIN" from=".SD .J_SD > .MISO" to=".J_LCD .J_HEADER > .SDO_MISO" />
      </>
    )}
    ...
    {sharedSpiTracePaths && storage && publishedMcuModule && !mcuHeaders ? (
      <>
        <trace name="SHARED_SPI_SCK" path={[".MCU .J_SPI > .SCK", ".MCU .MCU_CORE .U1 > .GPIO18", ".SD .J_SD > .CLK", ".J_LCD .J_HEADER > .SCK"]} />
        <trace name="SHARED_SPI_MOSI" path={[".MCU .J_SPI > .MOSI", ".MCU .MCU_CORE .U1 > .GPIO19", ".SD .J_SD > .MOSI", ".J_LCD .J_HEADER > .SDI_MOSI"]} />
        <trace name="SHARED_SPI_MISO" path={[".MCU .J_SPI > .MISO", ".MCU .MCU_CORE .U1 > .GPIO16", ".SD .J_SD > .MISO", ".J_LCD .J_HEADER > .SDO_MISO"]} />
        <trace name="SHARED_SPI_LCD_CS" path={[".MCU .J_SPI > .CS", ".MCU .MCU_CORE .U1 > .GPIO17", ".J_LCD .J_HEADER > .CS"]} />
      </>
    ) : (
      <trace name="LCD_CS" routingPhaseIndex={routeDisplayFirst ? 0 : undefined} from=".J_LCD .J_HEADER > .CS" to={chainSharedSignalTraces ? ".MCU .J_SPI > .CS" : publishedMcuModule ? (mcuHeaders ? ".MCU .J_LEFT > .pin18" : ".MCU .MCU_CORE .U1 > .GPIO17") : ".MCU .U1 > .GPIO17"} maxViaCount={mcuLocalSameLayerEscapes ? 0 : undefined} />
    )}
    <trace name="LCD_RST" from=".J_LCD .J_HEADER > .RESET" to={publishedMcuModule ? (mcuHeaders ? ".MCU .J_LEFT > .pin13" : ".MCU .MCU_CORE .U1 > .GPIO21") : ".MCU .U1 > .GPIO21"} />
    <trace name="LCD_DC" from=".J_LCD .J_HEADER > .DC_RS" to={publishedMcuModule ? (mcuHeaders ? ".MCU .J_LEFT > .pin14" : ".MCU .MCU_CORE .U1 > .GPIO20") : ".MCU .U1 > .GPIO20"} />
    {!sharedSpiTracePaths && (
      <>
        <trace name="LCD_MOSI" from=".J_LCD .J_HEADER > .SDI_MOSI" to={chainSharedSignalTraces ? ".MCU .J_SPI > .MOSI" : publishedMcuModule ? (mcuHeaders ? ".MCU .J_LEFT > .pin15" : ".MCU .MCU_CORE .U1 > .GPIO19") : ".MCU .U1 > .GPIO19"} />
        <trace name="LCD_SCK" from=".J_LCD .J_HEADER > .SCK" to={chainSharedSignalTraces ? ".MCU .J_SPI > .SCK" : publishedMcuModule ? (mcuHeaders ? ".MCU .J_LEFT > .pin17" : ".MCU .MCU_CORE .U1 > .GPIO18") : ".MCU .U1 > .GPIO18"} />
      </>
    )}
    <trace name="LCD_LED" from=".J_LCD .J_HEADER > .LED" to="net.V3V3" {...v3v3Label} />
    {!sharedSpiTracePaths && <trace name="LCD_MISO" from=".J_LCD .J_HEADER > .SDO_MISO" to={chainSharedSignalTraces ? ".MCU .J_SPI > .MISO" : publishedMcuModule ? (mcuHeaders ? ".MCU .J_LEFT > .pin19" : ".MCU .MCU_CORE .U1 > .GPIO16") : ".MCU .U1 > .GPIO16"} />}
    ...
    {copperIslands && (
      <copperpour
        ...
        outline={[
          ...
          { x: -17.3, y: 22.2 },
          { x: -18.5, y: 22.2 },
        ].map((point) => ({ x: (ldoFlipped ? -38 - point.x : point.x) + ldoOffset.x, y: (ldoFlipped ? 38 - point.y : point.y) + ldoOffset.y }))}
      />
    )}
    ...
    {copperIslands && !publishedMcuModule && (
      <copperpour
        ...
        outline={(compactCoreIsland ? [
          { x: 1.2, y: 23.8 }, { x: 3.3, y: 23.8 },
          { x: 3.3, y: 25.8 }, { x: 1.2, y: 25.8 },
        ] : [
          { x: -0.2, y: 23.8 }, { x: 2.9, y: 23.8 },
          { x: 2.9, y: 25.8 }, { x: -0.2, y: 25.8 },
        ]).map(({ x, y }) => ({
          x: x * Math.cos(coreIslandRotation * Math.PI / 180) - (y - 17) * Math.sin(coreIslandRotation * Math.PI / 180) + coreIslandOffsetX,
          y: 17 + x * Math.sin(coreIslandRotation * Math.PI / 180) + (y - 17) * Math.cos(coreIslandRotation * Math.PI / 180) + coreIslandOffsetY,
        }))}
      />
    )}
